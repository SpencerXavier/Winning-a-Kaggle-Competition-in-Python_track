...
label encoder provides distinct number for each category...缺點：the approach is harmful for linear model, but it works for tree-based models
...
step1: calculate the mean on the train, apply to the test
step2: split train into K folds. Calculate mean on (K-1) folds, apply to the K-th fold [out-of-fold] data-> prevent overfitting of the train set 為什麼????
step3: Add mean target encoded feature to the model
 
Mean encoded
-as for test data , we simply take train data and calculate mean target values for each category

Train encoding using out-of fold
- fold 1 用 fold2 data 算mean
- fold 2 用 fold1 data 算mean


practical guides
smoothing 
normally :we tool simple mean
...
smooth_mean_enci = target_sumi + a*global_mean / (ni+ a) ....  阿發通常是5-10
New categories : fill new categories in the test data with global_mean
